...
Experiment 1 elicited the masked repetition priming response to high- and low-frequency words. To ensure both the reliability and validity of our findings, meticulous attention was paid to determine the correct sample size and prime duration. The sample size able to guarantee an acceptable statistical power ($>80%$) was determined via a full-fledged power analysis. The details of the power analysis is available as supplemental material, which also includes the code used for the power simulations and the simulated datasets (<https://osf.io/r7d2q/>). Our analysis identified a sample size of 1,250 participants as optimal, ensuring robust statistical power especially for a raw FAE equal to or exceeding 10 ms —- a value closely matching the average FAE calculated from previous studies (cf. @tbl-litReview). In light of the limitations in the temporal accuracy and precision of current online stimulus delivery programs (observed in several pilots and previous published online studies conducted in our lab), we aimed for an intended sample size of 2,600, to enhance the likelihood of obtaining a sample size of at least 1,250 participants after applying all the necessary exclusion criteria to the data. Besides, any sample size exceeding 1,250 would only improve the precision of the estimated effect size.
...
Analysis scripts and an abridged version of the data collected can be found online (<https://osf.io/vn3r2>), and consisted of `r format(nrow(exp1_rawdata.sub), big.mark=",", scientific=F)` observations in total. The analysis consisted of three separate steps.
